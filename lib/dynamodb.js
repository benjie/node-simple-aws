...
// tidy removes any undefined keys
...
SimpleDynamoDB.prototype.updateItem = function updateItem() {
  var spec =
    [ ['tableName', Type.string]
    , ['hash', Type.key]
    , ['range', Type.optional(Type.key)]
    , ['updates', Type.oneOf(Type.objectOf(Type.array),Type.object)]
    , ['expected', Type.optional(Type.object)]
    , ['returnValues', Type.optional(Type.string)]
    , ['callback', Type.fn]
    ];
    ...
    for (var key in updates) {
      if (updates.hasOwnProperty(key)) {
        if (!Array.isArray(updates[key])) {
          updates[key] = ['PUT', updates[key]];
        }
        result[key] = { Action: updates[key][0] };
        if (typeof updates[key][1] !== 'undefined') {
          result[key].Value = formatValue(updates[key][1]);
        }
      }
    }
    return result;
  ...
  var params = tidy(
    { TableName: parsed.tableName
    , Key: formatKey(parsed)
    , AttributeUpdates: formatUpdates(parsed.updates)
    , Expected: formatExpected(parsed.expected)
    , ReturnValues: parsed.returnValues
    });
  this.client.updateItem(params, parsed.callback);
}
...
SimpleDynamoDB.prototype.batchWriteItem = function batchWriteItem() {
  ...
  function callback(err, res) {
    ...
    attempts++;
    console.warn("WARNING: Am retrying batchWriteItem to tables: "+Object.keys(res.UnproccessedItems).join(", "));
