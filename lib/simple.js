...
SimpleDynamoDB.prototype.getItem = function getItem() {
  ...
  var params =
    { TableName: parsed.tableName
    , Key:
      { HashKeyElement: keyElement(parsed.hash)
      , RangeKeyElement: keyElement(parsed.range)
      }
...
SimpleDynamoDB.prototype.putItem = function putItem() {
  ...
  if (parsed.expected) {
    expected = {};
    for (var k in parsed.expected) {
      if (parsed.expected.hasOwnProperty(k)) {
        if (parsed.expected[k] === false) {
          expected[k] = {Exists: false};
        } else {
          expected[k] = {Value: formatValue(parsed.expected[k])};
        }
      }
    }
  }
